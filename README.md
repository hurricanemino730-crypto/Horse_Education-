...
3. 参加者を切り替えながらレーダーチャート・参考指標テーブルを確認（基本情報・参考指標は直接編集可能）
4. 「総合評価を生成」で AI がカテゴリ別コメント（Being〜Acting）と全体まとめ文を生成（生成後も直接編集可能）
5. 2ページ目下部の「チームからのメッセージ」欄に、担当スタッフが参加者へのメッセージを手動で記入（AI生成ではない自由記述）
6. 「A4 PDFダウンロード」で個別PDF、「全員分PDF一括ダウンロード」でZIP出力
...
AIはJSON形式で「Being〜Actingの5カテゴリ別コメント」と「全体まとめ文」を生成します。出力後は総合評価ページ上でそのまま直接編集できます。
